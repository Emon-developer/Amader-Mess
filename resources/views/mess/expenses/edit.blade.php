<form action="{{ route('expenses.update',$expense->id) }}" method="post" id="create-form">
@csrf
@method('PUT')
<div class="row">
  <div class="col-md-12" style="padding-left: 25px;padding-right: 25px;">
    <div class="form-group row">
        ...
        <input type="text" name="date" id="date" class="form-control" value="{{ $expense->date }}" data-inputmask-alias="datetime" data-inputmask-inputformat="yyyy-mm-dd" data-mask="" im-insert="false">
    </div>
    <div class="form-group row">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody id="expenses_view">
                ...
                @foreach(json_decode($expense->details) as $key => $item)
                    <tr>
                        <td></td>
                        <td><input type="text" name="expenses_name[]" value="{{ $item->name }}" class="form-control" onchange="calculateTotal()" onkeyup="calculateTotal()"></td>
                        <td><input type="text" name="expenses_quantity[]" value="{{ $item->quantity }}" class="form-control"></td>
                        <td><input type="number" name="expenses_price[]" min="0" value="{{ $item->price }}" class="form-control text-right expenses" onchange="calculateTotal()" onkeyup="calculateTotal()"></td>
                        <td>
    ...
    <div class="form-group row">
      <button type="submit" class="btn btn-primary"><i class="fa fa-save"></i>&nbsp; Update Expenses</button>
    </div>
  </div>
</div>
</form>
